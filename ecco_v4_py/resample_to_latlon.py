...
def resample_to_latlon(orig_lons, orig_lats, orig_field,
    ...
    ###############################################################################################
    ## Modifications to allow extra (e.g. time, depth) dimensions (DS, 2023-04-20)    
    ...
    if n_extra_dims>0: # check if extra dimensions exist
        ...
        if orig_field.shape[0]!=orig_lats.shape[0]:
            ...
            if np.prod(extra_dim_shape)>1: # check if extra dimensions are meaningful (i.e. not just len 1)
